...
public sealed class GetShortlistedMembersQueryHandler(
    IUnitOfWork unitOfWork,
    ICurrentUserProvider currentUserProvider)
    : IRequestHandler<GetShortlistedMembersQuery, ErrorOr<ApiResponse<List<ApplyApplicantResponse>>>>
{
    public async Task<ErrorOr<ApiResponse<List<ApplyApplicantResponse>>>> Handle(GetShortlistedMembersQuery request, CancellationToken cancellationToken)
    {
        ...
        var shortlistedMembers = await unitOfWork.JobApplications.Entites()
            .Where(ja => ja.JobId == request.JobId
                         && ja.Status == (int)ApplicationStatus.Shortlisted)
            .Join(unitOfWork.Members.Entites(),
                ja => ja.MemberId,
                m => m.Id,
